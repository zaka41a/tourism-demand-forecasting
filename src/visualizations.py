...
def plot_time_series(df, target_col="bookings", title="Time Series Analysis"):
    ...
    fig.add_trace(
        go.Scatter(
            x=df["date"], y=df[target_col], name="Original", line=dict(color="#667eea", width=1)
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=df["date"], y=rolling_mean, name="30-day MA", line=dict(color="#f56565", width=2)
        ),
        row=2,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=df["date"], y=rolling_std, name="30-day Std", line=dict(color="#48bb78", width=2)
        ),
        row=2,
        col=1,
    )
...
def plot_residuals(y_true, y_pred):
    ...
    # Residuals Distribution
    fig.add_trace(
        go.Histogram(x=residuals, nbinsx=30, marker=dict(color="#48bb78"), name="Distribution"),
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=qq[0][0], y=qq[0][1], mode="markers", marker=dict(color="#ed8936"), name="Q-Q"
        ),
        row=2,
        col=1,
    )
    ...
    # Residuals over time
    fig.add_trace(
        go.Scatter(y=residuals, mode="lines", line=dict(color="#9f7aea"), name="Time Series"),
        row=2,
        col=2,
    )
...
def plot_seasonal_patterns(df, target_col="bookings"):
    ...
    """
    fig = make_subplots(rows=1, cols=2, subplot_titles=("Monthly Pattern", "Weekly Pattern"))

    # Monthly pattern
    monthly_avg = df.groupby("month")[target_col].mean()
    fig.add_trace(
        go.Bar(
            x=monthly_avg.index, y=monthly_avg.values, marker=dict(color="#667eea"), name="Monthly"
        ),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Bar(x=days, y=weekly_avg.values, marker=dict(color="#48bb78"), name="Weekly"),
        row=1,
        col=2,
    )
...
def create_forecast_plot(dates, actual, predicted, forecast_dates=None, forecast_values=None):
    ...
    fig.add_trace(
        go.Scatter(
            x=dates, y=actual, mode="lines", name="Actual", line=dict(color="#667eea", width=2)
        )
    )
...
def plot_performance_metrics(metrics_dict):
    ...
    fig.add_trace(
        go.Bar(
            x=metrics_names,
            y=metrics_values,
            marker=dict(color=metrics_values, colorscale="Viridis", showscale=True),
    ...
    fig.update_layout(
        title="Model Performance Metrics", xaxis_title="Metric", yaxis_title="Value", height=400
    )
